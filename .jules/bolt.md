...
## 2026-04-21 - Pre-compiling regex patterns
**Learning:** Re-compiling regex patterns inside frequently called methods (like parsing job description headers/companies) wastes CPU cycles on compilation overhead and memory on temporary objects.
**Action:** Hoist local regex lists to module-level constants and use `re.compile()` to instantiate them exactly once, using `.search()` directly on the pattern.
