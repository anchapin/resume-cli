...
## 2025-02-18 - FastAPI Blocking I/O
**Learning:** `async def` endpoints in FastAPI execute directly on the main event loop. If they contain synchronous blocking calls (like PDF compilation via subprocess, synchronous file I/O, or blocking `requests`/`anthropic` API calls), they starve the event loop and degrade concurrency.
**Action:** Always wrap such blocking calls with `anyio.to_thread.run_sync` (which is already available via FastAPI/Starlette) and use `functools.partial` when passing keyword arguments.
