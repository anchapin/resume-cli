...
## 2025-02-21 - [CRITICAL] LaTeX Injection RCE and DoS in PDF Compilation
**Vulnerability:** LaTeX compilers (pdflatex and pandoc fallback) were executed without `-no-shell-escape`, allowing LaTeX code to execute arbitrary shell commands if maliciously injected. Additionally, PDF compilation lacked timeouts, leading to potential Denial of Service (DoS) if compilation hangs.
**Learning:** Command execution involving complex parsers like LaTeX must always restrict dangerous features (like shell escapes) and enforce timeouts, even if the primary input is not directly user-provided, as upstream generators or AI could inject payloads.
**Prevention:** Always append `-no-shell-escape` for pdflatex and `--pdf-engine-opt=-no-shell-escape` for pandoc. Enforce a timeout in `process.communicate(timeout=30)` and safely clean up processes using `process.kill()` on `TimeoutExpired`.
