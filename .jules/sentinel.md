...
## 2025-02-28 - [Critical] Unrestricted LaTeX Compilation RCE and DoS
**Vulnerability:** PDF compilation via pdflatex and pandoc in PDFConverter and CoverLetterGenerator allowed arbitrary shell command execution due to missing -no-shell-escape flags, and could cause Denial of Service (DoS) due to unbounded compilation timeouts.
**Learning:** Even fallback PDF compilation engines or duplicated compilation routines can introduce critical RCE vectors if restricted execution flags and process timeouts are inconsistently applied.
**Prevention:** Always consistently enforce restricted mode and maximum process timeouts across all instances of external document compilation logic.
