...
## 2025-03-01 - [Critical] LaTeX Shell Escape Vulnerability in PDF Compilation
**Vulnerability:** PDF compilation via pdflatex and pandoc in PDFConverter and CoverLetterGenerator was missing the -no-shell-escape flag.
**Learning:** Without -no-shell-escape, malicious LaTeX content can execute arbitrary shell commands via the \write18 feature, leading to RCE.
**Prevention:** Always append -no-shell-escape (or --pdf-engine-opt=-no-shell-escape) to PDF compilation commands and enforce timeouts to mitigate Denial of Service (DoS) from hanging processes.
