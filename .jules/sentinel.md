...
## 2025-06-25 - [Critical] LaTeX Command Injection and DoS via Shell Escape
**Vulnerability:** PDF compilation via `pdflatex` and `pandoc` was executed without disabling shell escape (`-no-shell-escape`). This could allow arbitrary code execution (Command Injection/LFI) if user input containing `\write18` or similar LaTeX shell commands was compiled. Additionally, the subprocesses lacked timeouts, presenting a Denial of Service (DoS) risk from hanging compilation processes.
**Learning:** Subprocesses executing external tools like compilers must always be configured securely (e.g. disabling shell escapes) and bound by a timeout to prevent resource exhaustion and execution of arbitrary code.
**Prevention:** Always append `-no-shell-escape` to `pdflatex` and `--pdf-engine-opt=-no-shell-escape` to `pandoc` when generating PDFs from untrusted LaTeX sources. Always set a `timeout` on `subprocess.communicate()` calls.
