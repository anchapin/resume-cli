...
## 2025-03-09 - LaTeX and Pandoc RCE & DoS in Compilation Commands
**Vulnerability:** PDF compilation commands (`pdflatex` and `pandoc` fallback) executed untrusted `.tex` templates without sandboxing (`-no-shell-escape`) and without timeouts, exposing the system to Remote Code Execution (RCE) via `\write18` and Denial of Service (DoS) via infinite compilation loops.
**Learning:** Even fallback processes like `pandoc` can act as vectors for LaTeX-based exploits if they trigger a LaTeX engine under the hood. Timeouts without explicit `process.kill()` and repeated `communicate()` calls can lead to double-free bugs or zombie processes.
**Prevention:** Always enforce `-no-shell-escape` for all LaTeX compilation paths (including via `--pdf-engine-opt` for `pandoc`). Enforce subprocess timeouts and implement explicit process cleanup (`kill` and subsequent `communicate`) in `except subprocess.TimeoutExpired` blocks.
