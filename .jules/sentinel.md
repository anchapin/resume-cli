...
## 2024-05-18 - RCE in PDF compilation
**Vulnerability:** Found Remote Code Execution (RCE) vulnerability during PDF compilation in `cli/pdf/converter.py` and `cli/generators/cover_letter_generator.py` using `pdflatex` and `pandoc` without `-no-shell-escape`. Also missing execution timeout.
**Learning:** External processes for document rendering, particularly LaTeX and Pandoc, often default to allowing shell execution. If user input reaches these compilers, it can execute arbitrary commands on the system.
**Prevention:** Always use `-no-shell-escape` flags when executing `pdflatex` (directly) or `pandoc` (via `--pdf-engine-opt=-no-shell-escape`). Additionally, implement a `timeout` when waiting for `subprocess.Popen.communicate()` to prevent denial-of-service through infinite compilation loops.
