...
## 2026-07-12 - [Critical] LaTeX RCE and DoS in PDF Compilation
**Vulnerability:** The PDF compilation modules (`cli/pdf/converter.py` and `cli/generators/cover_letter_generator.py`) executed `pdflatex` and `pandoc` without disabling shell escapes or setting timeouts. This allowed potential Remote Code Execution (RCE) via malicious LaTeX commands (e.g., `\immediate\write18`) and Denial of Service (DoS) via compilation hangs.
**Learning:** Relying on default compiler settings for user-provided or AI-generated input is unsafe. Compilers like `pdflatex` have powerful capabilities that must be explicitly restricted.
**Prevention:** Always append `-no-shell-escape` (or equivalent) to `pdflatex` commands and enforce strict timeouts using `subprocess.communicate(timeout=X)`. If a timeout occurs, explicitly kill the process and perform a secondary `communicate()` to prevent resource leaks.
