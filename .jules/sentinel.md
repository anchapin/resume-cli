...
## 2026-07-01 - [CRITICAL] LaTeX Remote Code Execution via pdflatex/pandoc
**Vulnerability:** PDF compilation commands (pdflatex and pandoc) were executed without restricted execution flags (-no-shell-escape or --pdf-engine-opt=-no-shell-escape) and without subprocess timeouts. This could allow Remote Code Execution (RCE) via malicious LaTeX files and Denial of Service (DoS) via non-terminating compilations.
**Learning:** Always restrict shell access when executing LaTeX compilers and always implement explicit timeouts with process cleanup to avoid zombie processes.
**Prevention:** Apply -no-shell-escape for pdflatex and --pdf-engine-opt=-no-shell-escape for pandoc. Wrap process.communicate() with timeouts and catch subprocess.TimeoutExpired to kill the process and cleanly exit.
