...
## 2025-02-14 - Fix LaTeX Command Injection (RCE) in PDF Generation
**Vulnerability:** PDF compilation via `pdflatex` and `pandoc` lacked `-no-shell-escape` restrictions and timeouts, allowing Remote Code Execution (RCE) and Denial of Service (DoS) via maliciously crafted LaTeX payloads.
**Learning:** When invoking external document processors that have shell execution capabilities, the failure to explicitly disable these features leaves the system vulnerable to RCE. Subprocess calls without timeouts are vulnerable to DoS.
**Prevention:** Always append `-no-shell-escape` (or engine equivalents) to document compilation subprocesses, and wrap `communicate()` calls with explicit timeouts and process cleanup (`kill()`).
