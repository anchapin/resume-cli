...
## 2025-10-24 - [Critical] PDF Compilation RCE and DoS
**Vulnerability:** Subprocess calls to pdflatex and pandoc lacked the -no-shell-escape flag and timeout configurations in some modules, allowing potential remote code execution (RCE) via LaTeX injections and denial of service (DoS) via infinite loops.
**Learning:** Security controls like -no-shell-escape must be applied consistently across all invocation points of external engines. A missing argument in one module bypasses the security implemented in another.
**Prevention:** Centralize external process invocations or enforce strict security arguments and timeouts systematically for all pdflatex and pandoc executions.
