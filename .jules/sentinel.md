...
## 2026-07-16 - Prevent RCE and DoS in PDF Compilation
**Vulnerability:** LaTeX PDF compilation lacked `-no-shell-escape` flags and timeouts, allowing potential Remote Code Execution (RCE) via `\write18` shell escapes and Denial of Service (DoS).
**Learning:** Security controls implemented in one part of the codebase (`TemplateGenerator`) were omitted when the logic was extracted into utility modules.
**Prevention:** Consistently apply security flags and timeouts across all subprocess calls that process untrusted input.
