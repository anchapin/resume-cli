...
## 2025-02-24 - [HIGH] LaTeX RCE and DoS in PDF compilation
**Vulnerability:** Subprocess calls to pdflatex and pandoc lacked the -no-shell-escape flag and proper timeout handling, allowing potential RCE and DoS.
**Learning:** Subprocesses executing LaTeX engines require explicit flags to prevent arbitrary command execution and explicit timeouts to prevent zombie processes.
**Prevention:** Always use -no-shell-escape for pdflatex (and --pdf-engine-opt=-no-shell-escape for pandoc) and enforce timeouts with process kill cleanup when calling external PDF compilers.
