...
## 2025-02-24 - [Critical] RCE Vulnerability in PDF Compilation
**Vulnerability:** The pdflatex and pandoc commands allowed arbitrary shell execution because `-no-shell-escape` was missing.
**Learning:** When compiling untrusted or dynamically generated LaTeX, it is critical to disable shell execution to prevent Local File Inclusion and RCE.
**Prevention:** Always append `-no-shell-escape` (and `--pdf-engine-opt=-no-shell-escape` for pandoc) and handle subprocess timeouts.
