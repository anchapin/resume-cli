...
## 2026-07-29 - [Critical] RCE Vulnerability in PDF Compilation
**Vulnerability:** PDF compilation commands for pdflatex and pandoc lacked restricted execution flags (-no-shell-escape), allowing arbitrary shell command execution if LaTeX is injected.
**Learning:** Relying purely on template sanitization is insufficient defense; the compiler itself must be locked down to prevent shell escapes (defense-in-depth).
**Prevention:** Always use -no-shell-escape when compiling untrusted or dynamically generated LaTeX source.
