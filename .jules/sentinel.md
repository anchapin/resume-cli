...
## 2025-02-26 - [CRITICAL] LaTeX Subprocess Execution Missing Security Constraints
**Vulnerability:** PDF compilation via `subprocess.Popen` in `cli/pdf/converter.py` and `cli/generators/cover_letter_generator.py` lacked the `-no-shell-escape` flag for `pdflatex` (and `--pdf-engine-opt=-no-shell-escape` for `pandoc`) and did not enforce execution timeouts.
**Learning:** Even if templates escape LaTeX characters, running LaTeX compilation without disabling shell execution is inherently risky and can lead to RCE if an injection bypasses the escaping. Lack of timeouts also enables DoS attacks.
**Prevention:** Always enforce `-no-shell-escape` and strict `timeout` limitations on any subprocess executing LaTeX or Pandoc, and properly handle `subprocess.TimeoutExpired` exceptions.
