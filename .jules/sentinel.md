...
## 2025-02-26 - [Critical] RCE and DoS via Unrestricted LaTeX Compilation
**Vulnerability:** PDF compilation via `pdflatex` and `pandoc` in `CoverLetterGenerator` and `PDFConverter` was running without restricting shell execution and without a timeout.
**Learning:** Calling command-line document processors on untrusted/generated input without strict flags allows Remote Code Execution (RCE) via LaTeX's `\write18` functionality. Additionally, compilation processes without a timeout can hang indefinitely (Denial of Service).
**Prevention:** Always enforce the `-no-shell-escape` flag for `pdflatex` (and via `--pdf-engine-opt` for `pandoc`). Always wrap `subprocess.communicate()` with a strict `timeout` (e.g., 30s), gracefully killing the process if it expires.
