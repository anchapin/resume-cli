...
## 2025-02-19 - [CRITICAL] Remote Code Execution (RCE) in PDF Compilers
**Vulnerability:** The PDF compilation logic in `cli/pdf/converter.py` and `cli/generators/cover_letter_generator.py` invoked `pdflatex` and `pandoc` without disabling shell escapes, allowing potential RCE if malicious LaTeX code with shell commands was processed.
**Learning:** Subprocesses executing external tools like LaTeX compilers must explicitly disable unsafe features (like shell escapes) to prevent code execution vulnerabilities.
**Prevention:** Always append `-no-shell-escape` (or `--pdf-engine-opt=-no-shell-escape` for pandoc) to PDF compilation commands to enforce restricted execution.
