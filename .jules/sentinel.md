...
## 2024-06-27 - [Critical] LaTeX Compilation RCE via Subprocess Flags
**Vulnerability:** The PDF compilation modules (`cli/pdf/converter.py` and `cli/generators/cover_letter_generator.py`) did not consistently enforce the `-no-shell-escape` flag on `pdflatex` and `pandoc` subprocess commands, and lacked execution timeouts, leading to potential Remote Code Execution (RCE) and zombie process Denial of Service.
**Learning:** Security controls like restricted execution flags must be applied universally across all components that interface with the same external tool, and all subprocess calls require explicit timeouts and cleanup.
**Prevention:** Apply `-no-shell-escape` universally for LaTeX compilation commands and strictly enforce `timeout` and `kill()` cleanup blocks.
