...
## 2026-07-15 - [Critical] RCE and DoS in PDF Compilation
**Vulnerability:** PDF compilation commands (`pdflatex` and `pandoc`) in `cli/pdf/converter.py` and `cli/generators/cover_letter_generator.py` omitted the `-no-shell-escape` flag and lacked subprocess timeouts, exposing the application to Remote Code Execution (via malicious LaTeX `\write18`) and Denial of Service (via infinite compilation loops).
**Learning:** When invoking external compilation engines on user-provided or AI-generated content, security controls must be consistently applied across all execution paths, not just the primary generator.
**Prevention:** Always enforce restricted execution modes (e.g., `-no-shell-escape`) and strict timeouts for all subprocess calls handling untrusted input.
