...
## 2026-04-10 - [Critical] Server-Side Request Forgery (SSRF) in Job Parser
**Vulnerability:** The `JobParser.parse_from_url` method used `requests.get` to fetch arbitrary user-provided URLs without validating the target IP address. This could allow an attacker to bypass firewalls and make requests to internal services or cloud metadata endpoints.
**Learning:** Checking for "localhost" or "127.0.0.1" in the URL string is insufficient because attackers can use DNS rebinding or alternate encodings. The SSRF protection must resolve the IP address at the time of the request and validate the actual IP.
**Prevention:** Always parse the URL scheme, explicitly resolve the hostname to an IP using `socket.getaddrinfo`, and use the `ipaddress` module to ensure the IP is not restricted (private, loopback, link-local, etc.) before fetching. Furthermore, handle HTTP redirects manually to re-validate the target IP after every hop.
