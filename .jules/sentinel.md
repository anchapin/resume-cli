...
## 2026-07-30 - Fix RCE and DoS in PDF Compilation
**Vulnerability:** PDF compilation commands (pdflatex/pandoc) lacked restricted execution flags (`-no-shell-escape`) and timeouts, allowing arbitrary command execution and Denial of Service (DoS) via process hanging.
**Learning:** Security controls like timeouts and restricted shell environments must be uniformly applied across all modules invoking shell processes; refactored utilities (like `cli/pdf/converter.py`) often lose these constraints if not ported carefully.
**Prevention:** Always enforce `-no-shell-escape` for LaTeX processing and set explicit timeouts using `subprocess.communicate(timeout=30)` on all external process executions.
