...
## 2026-07-31 - [CRITICAL] LaTeX RCE via Shell Escape
**Vulnerability:** PDF compilation commands lacked restricted execution flags, allowing malicious LaTeX input to execute arbitrary system commands.
**Learning:** Default LaTeX compilation allows shell escapes which is dangerous if input is user-provided or AI-generated.
**Prevention:** Enforce `-no-shell-escape` in `pdflatex` and `--pdf-engine-opt=-no-shell-escape` in `pandoc` commands.
