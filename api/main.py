...
async def render_pdf(request: ResumeRequest):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            # SECURITY: Offload blocking operation to worker thread to prevent event loop starvation DoS
...
async def tailor_resume(request: TailorRequest):
    try:
        ...
        # SECURITY: Offload blocking AI call to worker thread to prevent event loop starvation DoS
    ...
    try:
        ...
        if request.format == "md" and "md" in outputs:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # SECURITY: Offload blocking PDF compilation to worker thread to prevent event loop starvation DoS
                pdf_created = await anyio.to_thread.run_sync(
...
async def render_resume_pdf(resume_id: str, variant: str = "base"):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # SECURITY: Offload blocking operations to worker threads to prevent event loop starvation DoS
        ...
        with open(resume_yaml_path, "w", encoding="utf-8") as f:
            await anyio.to_thread.run_sync(
                functools.partial(yaml.dump, yaml_data, f, default_flow_style=False)
            )
