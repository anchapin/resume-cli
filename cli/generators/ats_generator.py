...
_TECH_TERMS_PATTERN = re.compile(r"\b[a-z]+(?:\s+[a-z]+)?\b")
_SUMMARY_KEYWORDS_PATTERN = re.compile(r"\b[a-z]{2,}\b")

# Action verbs used for readability scoring
_ACTION_VERBS = [
    "developed",
    "implemented",
    "built",
        ...
        for job in experience:
            for bullet in job.get("bullets", []):
                if isinstance(bullet, dict):
                    ...
                    keywords.extend(_TECH_TERMS_PATTERN.findall(text))

        # Extract from summary
        summary = resume_data.get("summary", "")
        if summary:
            keywords.extend(_SUMMARY_KEYWORDS_PATTERN.findall(summary.lower()))
