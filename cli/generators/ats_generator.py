...
_SUMMARY_PATTERN = re.compile(r"\b[a-z]{2,}\b")
...
class ATSGenerator:
    ...
    def _check_readability(self, resume_data: Dict[str, Any]) -> ATSCategoryScore:
        ...
        action_verb_count = sum(1 for verb in _ACTION_VERBS if verb in text_lower)
...
Please extract the keywords:"""
    ...
    def _extract_resume_keywords(self, resume_data: Dict[str, Any]) -> List[str]:
        ...
        summary = resume_data.get("summary", "")
        if summary:
            keywords.extend(_SUMMARY_PATTERN.findall(summary.lower()))
