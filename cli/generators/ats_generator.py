...
_WORD_BOUNDARY_PATTERN = re.compile(r"\b[a-z]+(?:\s+[a-z]+)?\b")
_WORD_LENGTH_PATTERN = re.compile(r"\b[a-z]{2,}\b")

# Action verbs for readability checking
_ACTION_VERBS = [
    "developed",
    "implemented",
    "built",
        ...
        for job in experience:
            for bullet in job.get("bullets", []):
                if isinstance(bullet, dict):
                    ...
                    keywords.extend(_WORD_BOUNDARY_PATTERN.findall(text))

        # Extract from summary
        summary = resume_data.get("summary", "")
        if summary:
            keywords.extend(_WORD_LENGTH_PATTERN.findall(summary.lower()))
