...
QUANTIFIABLE_PATTERN = re.compile(r"\d+%|\$\d+|\d+\s*(users|customers|projects)")
ACRONYM_PATTERN = re.compile(r"\b[A-Z]{2,4}\b")
JSON_ARRAY_PATTERN = re.compile(r"\[.*\]", re.DOTALL)
TECH_TERMS_PATTERN = re.compile(r"\b[a-z]+(?:\s+[a-z]+)?\b")
...
class ATSGenerator:
    ...
    def _check_readability(self, resume_data: Dict[str, Any]) -> ATSCategoryScore:
        ...
        has_numbers = bool(QUANTIFIABLE_PATTERN.search(all_text))
        if has_numbers:
