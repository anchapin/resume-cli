...
_RESUME_KEYWORD_PATTERN = re.compile(r"\b[a-z]+(?:\s+[a-z]+)?\b")
...
class ATSGenerator:
    ...
    def _check_readability(self, resume_data: Dict[str, Any]) -> ATSCategoryScore:
        ...
        has_numbers = bool(_NUMBERS_PATTERN.search(all_text_lower))
        if has_numbers:
        ...
        # Note: acronyms must be matched against original case-preserved text
        ...
        for job in experience:
            for bullet in job.get("bullets", []):
                if isinstance(bullet, dict):
                    ...
                    keywords.extend(_RESUME_KEYWORD_PATTERN.findall(text))
