...
Return ONLY valid JSON, nothing else."""
    ...
    def _compile_pdf(self, output_path: Path, tex_content: str) -> bool:
        ...
        try:
            ...
            # SECURITY: Use -no-shell-escape to prevent arbitrary command execution (RCE) via \write18
            ...
            try:
                # SECURITY: Enforce 30s timeout to prevent DoS from infinite compilation loops
            ...
            if output_path.exists():
                ...
                # Fallback to pandoc
                try:
                    # SECURITY: Pass -no-shell-escape to the underlying LaTeX engine to prevent RCE
                    process = subprocess.Popen(
                        [
                            "pandoc",
                            str(tex_path),
