...
Return ONLY valid JSON, nothing else."""
    ...
    def _compile_pdf(self, output_path: Path, tex_content: str) -> bool:
        ...
        pdf_created = False
        try:
            # Added -no-shell-escape to prevent RCE vulnerabilities
            ...
            try:
                ...
                # Security: prevent DoS by killing stalled process
                process.kill()
                process.communicate()
                return False
            ...
            if output_path.exists():
                ...
                # Fallback to pandoc
                try:
                    # Added --pdf-engine-opt=-no-shell-escape to prevent RCE vulnerabilities
                    process = subprocess.Popen(
                        [
                            "pandoc",
                            str(tex_path),
