...
class ConnectionFinder:
    ...
    def find_previous_company_connections(self, company: str) -> List[Connection]:
        ...
        if not self.user_previous_companies:
            ...
            console.print("  Set LINKEDIN_PREVIOUS_COMPANIES environment variable (comma-separated).")
            return []
        ...
        for prev_company in self.user_previous_companies:
            if prev_company.strip():
                console.print(f"[dim]Searching for former {prev_company} employees at {company}...[/dim]")
...
Thank you!"""
            ...
            if conn.common_interests:
                talking_points.insert(0, f"Discuss common interests: {', '.join(conn.common_interests)}")
            
            suggestion = OutreachSuggestion(
                connection=conn,
                message_template=message,
                talking_points=talking_points,
                common_ground=", ".join(conn.common_interests) if conn.common_interests else conn.connection_degree,
            )
    ...
    def export_to_csv(self, connections: List[Connection], output_path: Path) -> None:
        ...
        with open(output_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Name', 'Role', 'Company', 'Connection Degree', 'Profile URL', 'Common Interests'])
