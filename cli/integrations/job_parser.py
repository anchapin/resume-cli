...
class JobParser:
    ...
    # Section header keywords to exclude - tuple used for fast prefix matching
    ...
    def _extract_items_from_text(self, text: str) -> List[str]:
        ...
        if not items:
            ...
            for line in lines:
                ...
                # Optimization: passing a tuple directly to startswith is significantly faster
                # than a generator expression. The `header + ":"` derivative check is also
                # removed since the base string match implies the derivative match.
