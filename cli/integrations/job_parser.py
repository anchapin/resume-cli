...
class JobParser:
    ...
    EXPERIENCE_LEVEL_PATTERN = re.compile(
    ...
    def _extract_sections_from_description(self, description: str) -> Tuple[List[str], List[str]]:
        ...
        req_match = self.REQ_PATTERN.search(description)
        resp_match = self.RESP_PATTERN.search(description)
    ...
    def _extract_items_from_text(self, text: str) -> List[str]:
        ...
        if not items:
            ...
            for line in lines:
                ...
                if line_lower.startswith(self.SECTION_HEADER_STARTS):
                    continue
    ...
    def _extract_job_type(self, html: str) -> Optional[str]:
        ...
        """
        match = self.JOB_TYPE_PATTERN.search(html)
        if match:
    ...
    def _extract_experience_level(self, html: str) -> Optional[str]:
        ...
        """
        match = self.EXPERIENCE_LEVEL_PATTERN.search(html)
        if match:
            return match.group(1).lower().replace("-", "-")
