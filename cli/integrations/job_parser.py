...
# Pre-compiled regex patterns for parsing job descriptions
_SALARY_PATTERNS = [
...
_COMMA_SEPARATED_PATTERN = re.compile(r",\s*(?=[A-Z])")
...
class JobParser:
    ...
    def _extract_sections_from_description(self, description: str) -> Tuple[List[str], List[str]]:
        ...
        if resp_start >= 0:
            ...
            for pattern in _NEXT_SECTION_PATTERNS:
                ...
                if next_match:
                    # Adjust because match.start() is relative to original string when using pos
    ...
    def _extract_items_from_text(self, text: str) -> List[str]:
        ...
        if not items:
            parts = _COMMA_SEPARATED_PATTERN.split(text)
    ...
    def _extract_list_by_keyword(self, html: str, keyword: Union[str, re.Pattern]) -> List[str]:
        ...
        Args:
            html: HTML content
            keyword: Keyword to search for (string or pre-compiled pattern)
