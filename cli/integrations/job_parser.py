...
# Compiling these once at module load avoids redundant compilation in loops and frequent method calls
_SALARY_PATTERNS = [
...
_SALARY_100K_PATTERN = re.compile(r"\d+k")
...
class JobParser:
    ...
    def _parse_generic(self, html: str) -> JobDetails:
        ...
        if not company:
            ...
            if meta_company:
                content_val = meta_company.get("content", "")
                company = str(content_val) if isinstance(content_val, str) else ""
    ...
    def _extract_salary_from_text(self, text: str) -> Optional[str]:
        ...
        for pattern in _SALARY_PATTERNS:
            ...
            if match:
                ...
                if "$" not in salary and _SALARY_100K_PATTERN.search(salary):
                    salary = "$" + salary
                return salary
    ...
    def _extract_list_by_keyword(self, html: str, keyword: Union[str, re.Pattern]) -> List[str]:
        ...
        Args:
            html: HTML content
            keyword: Keyword string to search for, or pre-compiled re.Pattern
