...
# Pre-compiled regex patterns for salary extraction
_SALARY_PATTERNS = [
...
_SALARY_CLEANUP_PATTERN = re.compile(r"\s+")
...
# Pre-compiled regex patterns for list items extraction
_BULLET_PATTERNS = [
...
class JobParser:
    ...
    def _parse_linkedin(self, html: str) -> JobDetails:
        ...
        if not company:
            ...
            company = self._extract_text_by_pattern(html, _GENERIC_COMPANY_PATTERN_1)
    ...
    def _parse_indeed(self, html: str) -> JobDetails:
        ...
        if not company:
            # Fallback patterns
            company = self._extract_text_by_pattern(html, _GENERIC_COMPANY_PATTERN_2)
    ...
    def _parse_generic(self, html: str) -> JobDetails:
        ...
        company = self._extract_text_by_pattern(html, _GENERIC_COMPANY_PATTERN_3)
        if not company:
        ...
        # Extract location
        location = self._extract_text_by_pattern(html, _GENERIC_LOCATION_PATTERN)
    ...
    def _extract_text_by_pattern(self, text: str, pattern: Union[str, re.Pattern]) -> Optional[str]:
        ...
        Args:
            text: Text to search
            pattern: Regex pattern (string or pre-compiled re.Pattern)
    ...
    def _extract_salary_from_text(self, text: str) -> Optional[str]:
        ...
        for pattern in _SALARY_PATTERNS:
            ...
            if match:
                ...
                salary = _SALARY_CLEANUP_PATTERN.sub(" ", salary.strip())
                if "$" not in salary and _SALARY_K_PATTERN.search(salary):
                    salary = "$" + salary
                return salary
    ...
    def _extract_sections_from_description(self, description: str) -> Tuple[List[str], List[str]]:
        ...
        req_match = _REQ_PATTERN.search(description)
        resp_match = _RESP_PATTERN.search(description)
