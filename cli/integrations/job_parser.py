...
_COMPANY_FALLBACK_PATTERN = re.compile(r'company["\s:]+([^"<>\n]+)', re.IGNORECASE)
...
class JobParser:
    ...
    def _parse_indeed(self, html: str) -> JobDetails:
        ...
        if not company:
            # Fallback patterns
            company = self._extract_text_by_pattern(html, _COMPANY_FALLBACK_PATTERN)
    ...
    def _extract_list_by_keyword(self, html: str, keyword: Union[str, re.Pattern]) -> List[str]:
        ...
        Args:
            html: HTML content
            keyword: Keyword to search for (string or compiled re.Pattern)
        ...
        search_pattern = (
            keyword if isinstance(keyword, re.Pattern) else re.compile(keyword, re.IGNORECASE)
        )

        # Find element containing the keyword
        for elem in soup.find_all(string=search_pattern):  # type: ignore[call-overload]
