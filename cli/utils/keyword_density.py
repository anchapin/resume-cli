...
_TITLE_PATTERNS = [
    ...
    re.compile(r"#\s*([^\n]+)", re.IGNORECASE | re.MULTILINE),  # Markdown headers
]
...
Please extract the keywords:"""
    ...
    def _count_keywords_in_resume(
        ...
        # Get all resume text and lower it once to avoid re.IGNORECASE overhead
        all_text = self._get_all_text(resume_data).lower()

        for keyword, _ in keywords:
            # Count occurrences using lowercased text and keywords
