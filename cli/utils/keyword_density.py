...
Please extract the keywords:"""
    ...
    def _count_keywords_in_resume(
        ...
        # Get all resume text and lowercase it once to avoid re.IGNORECASE overhead
        all_text_lower = self._get_all_text(resume_data).lower()

        for keyword, _ in keywords:
            # Count occurrences with lowercased text
            keyword_lower = keyword.lower()
            count = len(re.findall(rf"\b{re.escape(keyword_lower)}\b", all_text_lower))
