...
class TestParseBaseResume:
    ...
    def test_parse_base_resume_basic(self, temp_dir: Path):
        ...
        data = {"contact": {}, "professional_summary": {"base": "", "variants": {}}, "skills": {}, "experience": [], "education": [], "variants": {}}
    ...
    def test_parse_base_resume_with_credentials(self, temp_dir: Path):
        ...
        assert data["contact"]["name"] == "John Doe"
        assert data["contact"]["credentials"] == ["PhD"]
    ...
    def test_parse_base_resume_skills_with_bullets(self, temp_dir: Path):
        ...
        assert "cloud" in data["skills"]
        assert "Python" in data["skills"]["programming"]
        assert "AWS" in data["skills"]["cloud"]
    ...
    def test_parse_base_resume_experience_bullets_with_skills(self, temp_dir: Path):
        ...
        assert data["experience"][0]["bullets"][0]["text"] == "Built scalable APIs"
        assert data["experience"][0]["bullets"][0]["skills"] == ["Python"]
    ...
    def test_parse_base_resume_past_job(self, temp_dir: Path):
        ...
        assert data["experience"][0]["start_date"] == "2018-01"
        assert data["experience"][0]["end_date"] == "2020-01"
    ...
    def test_parse_base_resume_present_job(self, temp_dir: Path):
        ...
        assert data["experience"][0]["end_date"] is None
...
class TestAddDefaultVariants:
    ...
    def test_add_default_variants_ml_ai_focus(self):
        ...
        data = {"skills": {"programming": ["Python"], "ai_ml": ["TensorFlow"]}, "variants": {}}

        _add_default_variants(data)

        ml_ai = data["variants"]["v1.2.0-ml_ai"]
        assert "ai" in str(ml_ai.get("emphasize_keywords", [])).lower() or "ml" in str(ml_ai.get("emphasize_keywords", [])).lower()
