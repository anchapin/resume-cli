...
class TestPDFConverterSecurity(unittest.TestCase):
    ...
    def test_converter_pandoc_timeout(self, mock_popen):
        ...
        result = converter._compile_pandoc(Path("dummy.tex"), Path("output.pdf"), Path("."))
    ...
    def test_converter_pandoc_arguments(self, mock_popen):
        ...
        with patch.object(Path, "exists", return_value=True):
            converter._compile_pandoc(Path("dummy.tex"), Path("output.pdf"), Path("."))
...
class TestCoverLetterGeneratorSecurity(unittest.TestCase):
    def setUp(self):
        self.env_patcher1 = patch.dict(
            "os.environ", {"ANTHROPIC_API_KEY": "mock", "OPENAI_API_KEY": "mock"}
        )
        self.env_patcher1.start()
        ...
        self.dummy_resume = {"personal_information": {"name": "Test"}}

    def tearDown(self):
        self.env_patcher1.stop()
    ...
    def test_cover_letter_pdflatex_timeout(self, mock_popen):
        ...
        generator = CoverLetterGenerator(config=self.mock_config, resume_data=self.dummy_resume)
